...
async def capture_loop(
    ...
    def _handle_pkt(pkt):
        ...
        # Thread-safe push into the asyncio queue with safety wrap to handle QueueFull gracefully
        def _safe_put(item):
            try:
                queue.put_nowait(item)
            except asyncio.QueueFull:
                pass # Queue is full, drop packet silently to avoid event loop congestion

        loop.call_soon_threadsafe(_safe_put, cp)
