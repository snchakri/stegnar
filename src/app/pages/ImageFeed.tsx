...
export function ImageFeed() {
  ...
  const fetchImages = () => {
    ...
    fetch(`http://localhost:3001/api/images?${params}`)
      .then(r => r.json())
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-full">
                <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
                  ...
                  <div style={{ fontSize: 'var(--text-xs)', marginTop: 4 }}>curl http://localhost:3001/api/health</div>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  return (
                    <button key={img.id || img.sha256_hash} onClick={() => setSelected(img)}
                      ...
                      <div className="h-48 flex items-center justify-center" style={{ background: 'var(--bg-sidebar)' }}>
                        {img.minio_img_uri ? (
                          <img src={`http://localhost:9000/${img.minio_img_uri}`} alt="intercepted"
            ...
            <div className="flex-1 overflow-auto p-4 space-y-4">
              ...
              <div className="rounded-lg border flex items-center justify-center"
                ...
                {selected.minio_img_uri ? (
                  <img src={`http://localhost:9000/${selected.minio_img_uri}`} alt="intercepted"
                    style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                ) : (
