...
export function LogsPage() {
  ...
  const fetchLogs = () => {
    ...
    fetch(`http://localhost:3001/api/logs?${params}`)
      ...
      .then((data: LogRow[]) => {
        setLogs(data);
        const comps = Array.from(new Set(data.map(l => l.component).filter(Boolean)));
        setComponents(comps);
        setLoading(false);
      })
                  ...
                  <table className="w-full">
                    <thead>
                      <tr style={{ background: 'var(--bg-sidebar)', borderBottom: '1px solid var(--border-default)' }}>
                        {['TIMESTAMP', 'ENDPOINT', 'VERDICT', 'DETAILS'].map(h => (
                      ...
                      {logs.map((log, i) => {
                        ...
                        return (
                          <tr key={i} onClick={() => setSelectedLog(log)} className="cursor-pointer transition-colors table-row-hover"
                            ...
                            <td style={{ padding: '12px 14px' }}>
                              <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--text-secondary)', background: 'var(--bg-sidebar)', padding: '3px 7px', borderRadius: 4, display: 'inline-block', maxWidth: 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {typeof log.details === 'object' ? JSON.stringify(log.details).slice(0, 80) : String(log.details).slice(0, 80)}
                              </span>
                            </td>
                          </tr>
                        );
